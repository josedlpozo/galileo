...
class MagnifierView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(context,
    ...
    private fun drawGrid(canvas: Canvas) {
        ...
        while (x <= destinationPreviewRect.right) {
            canvas.drawLine(x, destinationPreviewRect.top.toFloat(), x, destinationPreviewRect.bottom.toFloat(), gridPaint)
            x += stepSize
        }
        var y = destinationPreviewRect.top + stepSize
        while (y <= destinationPreviewRect.bottom) {
            canvas.drawLine(destinationPreviewRect.left.toFloat(), y, destinationPreviewRect.right.toFloat(), y, gridPaint)
            y += stepSize
        }
    }
