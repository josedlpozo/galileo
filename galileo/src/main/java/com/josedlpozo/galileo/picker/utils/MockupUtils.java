    ...
    private static void saveMockup(Context context, Bitmap bmp, String fileName)
        ...
        if (bmp != null) {
            ...
            if (PORTRAIT_MOCKUP_FILENAME.equals(fileName)) {
                PreferenceUtils.MockPreferences.setPortraitMocupkOverlay(context, filePath);
            } else if (LANDSCAPE_MOCKUP_FILENAME.equals(fileName)) {
                PreferenceUtils.MockPreferences.setLandscapeMocupkOverlay(context, filePath);
            }
        } else {
            if (PORTRAIT_MOCKUP_FILENAME.equals(fileName)) {
                if (new File(filePath).delete()) {
                    PreferenceUtils.MockPreferences.setPortraitMocupkOverlay(context, "");
                }
            } else if (LANDSCAPE_MOCKUP_FILENAME.equals(fileName)) {
                if (new File(filePath).delete()) {
                    PreferenceUtils.MockPreferences.setLandscapeMocupkOverlay(context, "");
                }
            }
        }
    }
