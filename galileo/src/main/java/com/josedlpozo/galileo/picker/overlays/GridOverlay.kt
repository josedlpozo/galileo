...
class GridOverlay : ServiceFloatItem() {
    ...
    override fun getPersistentNotification(): Notification {
        val actionIsHide = DesignerTools.gridOverlayOn(this)
        val pi = PendingIntent.getBroadcast(this, 0, Intent(if (actionIsHide) ACTION_HIDE_OVERLAY else ACTION_SHOW_OVERLAY), 0)
        val builder: Notification.Builder = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            Notification.Builder(this, channel)
        } else {
            Notification.Builder(this)
        }
        val text = getString(if (actionIsHide) R.string.notif_content_hide_grid_overlay else R.string.notif_content_show_grid_overlay)
        builder.setSmallIcon(if (actionIsHide) R.drawable.ic_qs_grid_on else R.drawable.ic_qs_grid_off)
    ...
    private fun handleVisibility() {
        ...
        if (DesignerTools.gridOverlayOn(context)) show() else hide()
    }
