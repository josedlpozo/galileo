...
class BrowserPresenter(private val view: BrowserContract.View) {
    ...
    fun requestForContentUpdate(dynamicRealm: DynamicRealm?, @BrowserContract.DisplayMode displayMode: Int) {
        ...
        if (displayMode == BrowserContract.DisplayMode.REALM_CLASS) {
            realmModelClass = DataHolder.getInstance().retrieve(DATA_HOLDER_KEY_CLASS) as Class<out RealmModel>?
            ...
            val dynamicRealmObject = DataHolder.getInstance().retrieve(DATA_HOLDER_KEY_OBJECT) as DynamicRealmObject?
            val field = DataHolder.getInstance().retrieve(DATA_HOLDER_KEY_FIELD) as Field?
            if (dynamicRealmObject != null && field != null) {
                ...
                if (Utils.isParametrizedField(field)) {
                    realmModelClass = (field.genericType as ParameterizedType).actualTypeArguments[0] as Class<out RealmObject>
                }
            }
        }
    ...
    fun onInformationSelected() {
        map(Option.fromNullable(dynamicRealm), Option.fromNullable(realmModelClass)) {
            if (!it.a.isClosed) {
                view.showInformation(it.a.where(it.b.simpleName).count())
            }
        }.fix()
    }
    ...
    fun onRowSelected(realmObject: DynamicRealmObject) {
        if (this.realmModelClass != null) {
            DataHolder.getInstance().save(DATA_HOLDER_KEY_OBJECT, realmObject)
    ...
    private fun getFieldsList(dynamicRealm: DynamicRealm, realmModelClass: Class<out RealmModel>?): List<Field> {
